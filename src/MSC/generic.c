/* ___________----------------_________________  NOTE ________________----------------------__________________________*/
/* This c-source haas no use in teh compilation of tool .It is effectively "inert". The reason of its presence is not clear */
/* Maybe since all the benchmarks are effectively asm volatile codes only (either validation one or embedded in the code) so in case if some machine or c compiler is unable to use asm volatile constructs then this file has to be attached  */
